...
Aktywne joby stacji mogą mieć status: `queued`, `leased`, `running`, `retrying`, `done`, `failed`, `cancelled`, `expired`, `dead_letter`. Stacje nie pobierają jobów przez zwykły `select status=queued`; używają RPC `claim_workstation_jobs(p_workstation_id, p_limit, p_lease_seconds)`, które atomowo ustawia `leased`, `lease_owner` i `lease_expires_at` z `FOR UPDATE SKIP LOCKED`. Przy błędzie agent zapisuje `retrying` z backoffem albo `dead_letter`, gdy wyczerpie `max_attempts`. Przed każdym claimem stacja woła `release_expired_workstation_jobs(p_workstation_id)`, dzięki czemu joby porzucone przez padłą stację (lease minął bez aktualizacji) wracają do `queued` lub trafiają do `dead_letter` po przekroczeniu `max_attempts`. Każdy job dostaje `idempotency_key` w formacie `task:<taskId>:workstation:<workstationId>:attempt:<n>`, więc retry kierownika nie tworzy duplikatów.
